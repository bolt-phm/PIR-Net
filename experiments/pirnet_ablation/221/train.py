...
def train_epoch(model, loaders, criterion, optimizer, device, epoch, config, writer=None, scheduler=None, warmup_epochs=0, scaler=None, amp_enabled=False):
    model.train()
...
def validate(model, loaders, criterion, device, epoch, writer=None, amp_enabled=False):
    model.eval()
    total_loss, total_acc = 0, 0
    total_batches = sum(len(l) for l in loaders.values())
...
def main():
    ...
    for epoch in range(start_epoch, config['train']['epochs']):
        ...
        train_loss, train_acc = train_epoch(
            ...
            writer, scheduler, warmup_epochs=warmup_epochs, scaler=scaler, amp_enabled=amp_enabled
        )

        val_loss, val_acc = validate(model, val_loaders, criterion, device, epoch, writer, amp_enabled=amp_enabled)
