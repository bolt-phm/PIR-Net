...
def augment_online(img, sig, config):
    ...
    # For signal-only baselines we may disable pseudo-image generation entirely.
    # In that case, skip image augmentations to avoid unnecessary overhead.
    if bool(config.get('data', {}).get('disable_pseudo_image', False)):
        img_cfg = {}
...
class SmartResampledDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            if bool(self.config.get('data', {}).get('disable_pseudo_image', False)):
                h, w = tuple(self.config['data'].get('image_size', [224, 224]))
