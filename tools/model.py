...
def build_model(config):
    ...
    if name == "signal_inceptiontime":
        return InceptionTimeSignalNet(config)
    if name == "signal_resnet1d":
        return ResNet1DSignalNet(config)
    raise ValueError(f"Unknown model.name: {name}")


# ==============================================================================
# 6. Mainstream Fault-Diagnosis Baselines (Signal-only)
#    These models keep the same forward(img, sig) signature for compatibility
#    with the existing train/eval pipeline, but ignore the pseudo-image input.
# ==============================================================================

def _ensure_sig_3d(sig: torch.Tensor) -> torch.Tensor:
    # expected: (B, 1, L)
    if sig.dim() == 2:
...
class WDCNNSignalNet(nn.Module):
    """
    WDCNN-style 1D CNN baseline (widely used in bearing/machinery fault diagnosis).
    Reference architecture style: wide first-layer kernel + progressive pooling.
    """

    def __init__(self, config):
        super().__init__()
        num_classes = int(config["data"]["num_classes"])
        dropout = float(config.get("train", {}).get("dropout", 0.3))
    ...
    def forward(self, image_data, signal_data):
        sig = _ensure_sig_3d(signal_data)
        x = self.features(sig)
        return self.classifier(x)


class InceptionTimeSignalNet(nn.Module):
    """
    InceptionTime-like baseline for time-series classification.
    Uses the existing MultiScaleConvBlock as an Inception-style module.
    """
    ...
    def __init__(self, config):
        ...
        in_ch = 64
        for _ in range(6):
            blocks.append(MultiScaleConvBlock(in_ch, 128, stride=1))
            blocks.append(nn.BatchNorm1d(128))
...
class ResNet1DSignalNet(nn.Module):
    """
    ResNet1D baseline (common strong baseline for vibration time-series).
    """
    ...
    def __init__(self, config):
        ...
        self.layer1 = nn.Sequential(_ResBlock1D(64, 64, stride=1), _ResBlock1D(64, 64, stride=1))
        self.layer2 = nn.Sequential(_ResBlock1D(64, 128, stride=2), _ResBlock1D(128, 128, stride=1))
        self.layer3 = nn.Sequential(_ResBlock1D(128, 256, stride=2), _ResBlock1D(256, 256, stride=1))
